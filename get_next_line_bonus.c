...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/22 12:21:23 by aadnane           #+#    #+#             */
/*   Updated: 2022/03/24 19:24:09 by aadnane          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (n < 0 || (s[fd] == NULL && n == 0))
	{
		free(buf);
		return (NULL);
	}
	buf[n] = '\0';
	buf = ft_checkline (fd, buf, n, &s[fd]);
	if (!buf)
		return (NULL);
	line = makeline(buf, &s[fd]);
	free(buf);
	return (line);
}
